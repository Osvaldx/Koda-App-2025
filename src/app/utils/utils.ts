...
export const getFormErrors = (formGroup: FormGroup, controlName?: string): string => {
  ...
  const getError = (controlName: string) => {
    ...
    const errorMessages: Record<string, string> = {
      ...
      min: `'${formName}' debe ser mayor o igual a ${
        formGroup.get(controlName)?.errors?.['min']?.min
      }`,
      max: `'${formName}' debe ser menor o igual a ${
        formGroup.get(controlName)?.errors?.['max']?.max
      }`,
      ...
      invalidDomain: 'El dominio del correo electrónico tiene más de 24 caracteres',
    };
